...
describe("POST /discs", function(){
    ...
    test("works for admin: creates disc", async function(){
        const resp = await request(app).post("/discs").send(newDisc).set("authorization", `Bearer ${adminToken}`);
        expect(resp.statusCode).toBe(201);
        expect(resp.body).toEqual({Created: newDisc});
    })
    test("unauth for non-admins", async function(){
        const resp = await request(app).post("/discs").send(newDisc).set("authorization", `Bearer ${u1Token}`);
        expect(resp.statusCode).toBe(401);
    })
    test("unauth for anon user", async function(){
...
describe("DELETE /discs", function(){
    test("works for admin: disc is deleted", async function(){
        const resp = await request(app).delete("/discs/12345").set("authorization", `Bearer ${adminToken}`);
    ...
    test("unauth for non-admins", async function(){
        const resp = await request(app).delete("/discs/12345").set("authorization", `Bearer ${u1Token}`);
        expect(resp.statusCode).toBe(401);
    })
    test("unauth for anon user", async function(){
