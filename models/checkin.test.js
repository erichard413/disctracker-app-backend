...
describe("works: should update check in", function() {
    test("should update check in", async function() {
        const checkins = await CheckIn.getAll();
        await CheckInMock.updateCheckIn(checkins[checkins.length-1].id, {courseName: "updatedCourseName", city: "updatedCity", state: "updatedState", zip: "11111"});
    ...
    test("should throw error if no city, state & zip are provided", async function(){
        const checkins = await CheckIn.getAll();
        try {
            await CheckInMock.updateCheckIn(checkins[checkins.length-1].id, {courseName: "test"});
        } catch(err) {
