...
describe("works: get all discs", function () {
  test("should retrieve all discs", async function () {
    const result = await Disc.getDiscs();
    expect(result.length).toEqual(3);
    expect(result[0].id).toBe("12345");
    expect(result[1].name).toBe("Name2");
    expect(result[2].manufacturer).toBe("Manufacturer3");
  });
  test("should retrieve disc with id of 12345", async function () {
    const data = { id: "12345" };
    const result = await Disc.getDiscs(data);
    expect(result[0].id).toBe("12345");
  });
  test("should retrieve disc with Manufacturer3", async function () {
    const data = { manufacturer: "Manufacturer3" };
    const result = await Disc.getDiscs(data);
    expect(result[0].manufacturer).toBe("Manufacturer3");
  });
});
...
describe("works: creates disc", function () {
  test("should create a new disc", async function () {
    ...
    expect(result).toEqual({ ...newDisc, imgUrl: null });
  });
});
